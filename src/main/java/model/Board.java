...
public class Board {
     ...
     * less than 0, the board at the location will be set to {@link #EMPTY}.
     *
     * @param tileIndex the index of the black tile (from 0 to 31 inclusive).
     * @param id        the new ID to set the black tile to.
     * @see {@link #set(int, int, int)}, {@link #EMPTY}, {@link #BLACK_CHECKER},
     * {@link #WHITE_CHECKER}, {@link #BLACK_KING}, {@link #WHITE_KING}
     */
    public void set(int tileIndex, int id) {
        ...
        for (int i = 0; i < state.length; i++) {
            ...
            this.state[i] = setBit(state[i], tileIndex, set);
        }
    }
    ...
    public int get(int tileIndex) {
        ...
        return getBit(state[0], tileIndex) * 4 + getBit(state[1], tileIndex) * 2
                + getBit(state[2], tileIndex);
    }
     ...
     *
     * @param tileIndex the index of the black tile to convert to a point.
     * @return the (x, y) point corresponding to the black tile index or the
     * point (-1, -1) if the index is not between 0 - 31 (inclusive).
     * @see {@link #toTileIndex(int, int)}, {@link #toTileIndex(Point)}
     */
    public static Point toPoint(int tileIndex) {
        int y = tileIndex / 4;
        int x = 2 * (tileIndex % 4) + (y + 1) % 2;
        return !isValidTileIndex(tileIndex) ? new Point(-1, -1) : new Point(x, y);
    }
     ...
     * tile.
     * @see {@link #toTileIndex(Point)}, {@link #toPoint(int)}
     */
    public static int toTileIndex(int x, int y) {
     ...
     * @return the index of the black tile or -1 if the point is not a black
     * tile.
     * @see {@link #toTileIndex(int, int)}, {@link #toPoint(int)}
     */
    public static int toTileIndex(Point p) {
        return (p == null) ? -1 : toTileIndex(p.x, p.y);
    }
     ...
     * @param tileIndex    the bit to update (from 0 to 31 inclusive).
     ...
     */
    public static int setBit(int target, int tileIndex, boolean set) {
     ...
     * @param tileIndex the bit to get (from 0 to 31 inclusive).
     * @return 1 if and only if the specified bit is set, 0 otherwise.
     * @see {@link #setBit(int, int, boolean)}
     */
    public static int getBit(int target, int tileIndex) {
        ...
        return (target & (1 << tileIndex)) != 0 ? 1 : 0;
    }
     ...
     * @see {@link #middlePoint(int, int)}, {@link #middlePoint(int, int, int, int)}
     */
    public static Point middlePoint(Point p1, Point p2) {
        ...
        return 
                middlePoint(p1.x, p1.y, p2.x, p2.y);
    }

    /**
     * Gets the middle point on the checker board between two points.
     *
     * @param tileIndex1 the index of the first point (from 0 to 31 inclusive).
     * @param tileIndex2 the index of the second point (from 0 to 31 inclusive).
     * @return the middle point between two points or (-1, -1) if the points
     * are not on the board, are not distance 2 from each other in x and y,
     * or are on a white tile.
     * @see {@link #middlePoint(Point, Point)}, {@link #middlePoint(int, int, int, int)}
     */
    public static Point middlePoint(int tileIndex1, int tileIndex2) {
        return middlePoint(toPoint(tileIndex1), toPoint(tileIndex2));
    }
     ...
     * are not on the board, are not distance 2 from each other in x and y,
     * or are on a white tile.
     * @see {@link #middlePoint(int, int)}, {@link #middlePoint(Point, Point)}
     */
    public static Point middlePoint(int x1, int y1, int x2, int y2) {
     ...
     */
    public static boolean isValidTileIndex(int testIndex) {
