...
public class Game {
     ...
     * @see {@link #makeMove(int, int)}
     */
    public boolean makeMove(Point start, Point end) {
        if (start == null || end == null) {
            return false;
        }
        return makeMove(Board.toTileIndex(start), Board.toTileIndex(end));
    }
    ...
    public boolean makeMove(int startIndex, int endIndex) {
        ...
        int midIndex = Board.toTileIndex(middle);
        ...
        boolean midValid = Board.isValidTileIndex(midIndex);
        if (midValid) {
