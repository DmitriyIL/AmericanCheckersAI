...
public class MoveLogic {
	...
	public static boolean isValidMove(Board board, boolean isP1Turn,
		...
		// Basic checks
		if (board == null || !Board.isValidTileIndex(startIndex) ||
				!Board.isValidTileIndex(endIndex)) {
			return false;
		} else if (startIndex == endIndex) {
			return false;
		} else if (Board.isValidTileIndex(skipIndex) && skipIndex != startIndex) {
			return false;
		}
	...
	private static boolean validateIDs(Board board, boolean isP1Turn,
		...
		// Check the middle
		Point middle = Board.middlePoint(startIndex, endIndex);
		int midID = board.get(Board.toTileIndex(middle));
	...
	public static boolean isSafe(Board board, Point checker) {
		...
		for (Point p : check) {
			...
			int endIndex = Board.toTileIndex(new Point(p.x + dx, p.y + dy));
